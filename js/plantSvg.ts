/**
 * generatePlantSvg — produce a consistent 320×200 botanical illustration SVG
 * string for a given plant growth form and colour.
 *
 * The SVG is self-contained and suitable for use as a data URI, inline embed,
 * or standalone file. All gradient IDs are namespaced by a hash of the inputs
...
// Simple deterministic hash for gradient ID namespacing
...
const W = 320;
const H = 200;
const GND = 172; // y-coordinate of substrate surface
...
function waterBg(ns: string): string {
  return `<defs>
  <linearGradient id="wbg-${ns}" x1="0" y1="0" x2="0" y2="1">
    <stop offset="0" stop-color="#071912"/>
    <stop offset="1" stop-color="#0D2E1E"/>
  </linearGradient>
</defs>
<rect width="${W}" height="${H}" fill="url(#wbg-${ns})"/>
<rect x="0" y="${GND}" width="${W}" height="${H - GND}" fill="#0B1E15"/>
<path d="M0 ${GND} Q80 ${GND - 6} 160 ${GND} Q240 ${GND + 6} 320 ${GND}" stroke="#163526" stroke-width="1" fill="none"/>`;
}
...
function carpetIllustration(c: string, light: string, dark: string): string {
  let g = "";
  // back row (smaller, darker, partially hidden)
...
function epiphyteIllustration(c: string, light: string, dark: string): string {
  ...
  const leafData = [
    { x: 122, a: -78, len: 58, w: 13 },
    { x: 138, a: -52, len: 74, w: 16 },
    { x: 158, a: -86, len: 84, w: 18 },
    { x: 172, a: -62, len: 68, w: 15 },
...
function mossIllustration(c: string, light: string, dark: string): string {
  let g = "";
  // layered clumps — dark base, mid colour, light tips
  const base = [
    [138, 155, 42, 22],
    [174, 153, 40, 20],
    [154, 145, 38, 19],
...
function floatingIllustration(c: string, light: string, dark: string): string {
  ...
  const pads = [
    { cx: 118, cy: 33, rx: 40, ry: 22 },
    { cx: 192, cy: 30, rx: 33, ry: 18 },
    { cx: 156, cy: 26, rx: 27, ry: 15 },
  ];

  pads.forEach(({ cx, cy, rx, ry }) => {
    // root trail
    g += `<line x1="${cx}" y1="${cy + ry}" x2="${cx + (Math.random() < 0.5 ? 6 : -6)}" y2="${GND - 8}" stroke="${dark}" stroke-width="1" stroke-dasharray="4 4" opacity="0.35"/>`;
    // pad
    g += `<ellipse cx="${cx}" cy="${cy}" rx="${rx}" ry="${ry}" fill="${c}"/>`;
    // radial veins
    for (let a = 0; a < 360; a += 45) {
      const rad = (a * Math.PI) / 180;
      g += `<line x1="${cx}" y1="${cy}" x2="${(cx + Math.cos(rad) * rx * 0.9).toFixed(1)}" y2="${(cy + Math.sin(rad) * ry * 0.9).toFixed(1)}" stroke="${dark}" stroke-width="0.8" opacity="0.4"/>`;
    }
    // highlight
...
function rhizomeIllustration(c: string, light: string, dark: string): string {
  ...
  const nodes = [100, 120, 140, 160, 180, 200, 218];
  nodes.forEach((x, i) => {
    const h = 50 + (i % 3) * 20;
...
/**
 * Generate a 320×200 SVG string illustrating a plant's growth form.
 *
 * @param growthForm  One of the canonical growthForm enum values.
 * @param colorHex    Hex colour string (e.g. "#2f5d34") for the plant body.
 * @returns           A complete `<svg>` string ready to embed or use as a data URI.
 */
export function generatePlantSvg(
  ...
  switch (growthForm) {
    case "stem":
      body = stemIllustration(colorHex, light, dark);
      break;
    case "rosette":
      body = rosetteIllustration(colorHex, light, dark);
      break;
    case "carpeting":
      body = carpetIllustration(colorHex, light, dark);
      break;
    case "epiphyte":
      body = epiphyteIllustration(colorHex, light, dark);
      break;
    case "moss":
      body = mossIllustration(colorHex, light, dark);
      break;
    case "floating":
      body = floatingIllustration(colorHex, light, dark);
      break;
    case "rhizome":
      body = rhizomeIllustration(colorHex, light, dark);
      break;
    default:
      body = rosetteIllustration(colorHex, light, dark);
  }
...
/**
 * Convenience overload that accepts a full Plant object.
 */
export function plantToSvg(plant: Pick<Plant, "growthForm" | "colorHex">): string {
  return generatePlantSvg(plant.growthForm, plant.colorHex);
}
